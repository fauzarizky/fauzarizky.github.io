...
export default function Navbar({ children }) {
  ...
  return (
    <div className="min-h-screen bg-[#FBF9F4] font-sans text-[#272343] transition-colors duration-300 dark:bg-[#211D3A] dark:text-[#F5F1E8]">
      <nav
        data-test="navbar"
        className="sticky top-0 z-50 flex items-center justify-between bg-[#272343] px-6 py-[22px] dark:bg-[#14111F] md:px-16"
      >
        <a data-test="logo" href="#home" className="font-mono text-[22px] font-bold text-[#FFD803] no-underline">
          rzky()
        </a>
        ...
        <div className="hidden items-center gap-8 md:flex">
          {LINKS.map((link) => (
            <a
              key={link.href}
              data-test={`nav-link-${link.label.toLowerCase()}`}
              href={link.href}
          ...
          >
            <div className="flex flex-col items-end gap-4 pb-6 pt-2">
              {LINKS.map((link) => (
                <a
                  key={link.href}
                  data-test={`mobile-nav-link-${link.label.toLowerCase()}`}
                  href={link.href}
                  onClick={(e) => handleNavClick(e, link.href)}
                  className="font-mono text-sm text-white/85 no-underline transition-opacity hover:opacity-100 cursor-pointer"
                >
